...
function Grid() {
  ...
  for (let k = 0; k < 25; k++) {
    if ((k >= 11 && k <= 12) || (k >= 8 && k <= 9)) {
      for (let i = 0; i < 20; i++) {
        for (let j = 0; j < 20; j++) {
          map.push(new Land(k + ';' + (i * 20) + ';' + j, 'park', i, j, '', null))
        }
      }
    } else {
      for (let i = 0; i < 20; i++) {
        let x = (k + 1) * (i + 1)
        for (let j = 0; j < 20; j++) {
          let price = Math.floor(Math.random() * 16) + 5
  ...
  // console.log("🚀 ~ file: Grid.js ~ line 90 ~ Grid ~ map", map)

  let newMap = []

  for (let i = 0; i < map.length; i++) {
    let temp = Math.floor((Math.floor(i / 20) * 400) % 2000) + i % 20 + 20 * Math.floor((Math.floor(i / 20) * 400) / 2000) + 1600 * Math.floor(i / 2000)
  ...
  // let ul = document.getElementById("landsMap");
  ...
  // // return (
  // ul.innerHTML = map.map(land => {
  //   return `<div className="land">
  //         <button className="btn btn-primary">${land.id}</button>
  //     </div>`;
  // }).join("")
  // // )
